...
class DescargarReporteTActivity : AppCompatActivity() {
    ...
    private lateinit var adapter: EstudianteAdapterT
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter = EstudianteAdapterT(listaDocumentos) { doc ->
            val datos = doc.data ?: return@EstudianteAdapterT
            val nombre = datos["Datos de Identificacion"]?.let { (it as Map<*, *>)["apellidos_familia"] } ?: "Sin nombre"
            val idEstudiante = datos["Datos de Identificacion"]?.let { (it as Map<*, *>)["idEstudiante"] } ?: ""
    ...
    private fun buscarEncuestas(idAlumno: String) {
        db.collection("Tarjeta_Salud")
            .whereEqualTo("Datos de Identificacion.idEstudiante", idAlumno)
            .get()
    ...
    private fun crearPDF(datos: Map<String, Any>, nombrePaciente: String, idEstudiante: String) {
        val documento = Document(PageSize.A4, 40f, 40f, 50f, 50f) // Márgenes
        val nombreArchivo = "TarjetaSalud_${nombrePaciente.replace(" ", "_")}_${System.currentTimeMillis()}.pdf"
        ...
        try {
            ...
            if (outputStream != null) {
                ...
                datosFiltrados.forEach { (key, value) ->
                    when {
                        // Secciones importantes en formato de tabla
                        key.equals("Datos de Identificacion", ignoreCase = true) ||
                                key.equals("Alimentación", ignoreCase = true) ||
                                key.equals("Datos_Condicionantes", ignoreCase = true) ||
                                key.equals("Datos_familiares", ignoreCase = true) ||
                                key.equals("Saneamiento_Basico", ignoreCase = true) ||
                                key.equals("cierre_encuesta", ignoreCase = true) ||
                                key.equals("estilos_vida", ignoreCase = true) -> {
                            agregarSeccionConTabla(documento, key, value)
                        }

                        // Otras secciones en formato de lista
                        else -> {
    ...
    private fun agregarEncabezado(documento: Document, nombrePaciente: String, idEstudiante: String) {
        // Título principal
        val titulo = Paragraph("TARJETA DE SALUD FAMILIAR", fuenteTitulo)
    ...
    private fun agregarSeccionConTabla(documento: Document, titulo: String, value: Any) {
        ...
        parrafoTitulo.spacingBefore = 15f
        ...
        when (value) {
            is Map<*, *> -> {
                ...
                // Encabezado de tabla
                val celdaHeaderCampo = PdfPCell(Phrase("Campo", fuenteTextoBold))
                ...
                celdaHeaderCampo.phrase = Phrase("Campo", fuenteBlanca)
